...
| 2 | Touch & Interaction | CRITICAL | `ux` | Min size 44أ—44px, 8px+ spacing, Loading feedback | Reliance on hover only, Instant state changes (0ms) |
...
- `touch-target-size` - Min 44أ—44pt (Apple) / 48أ—48dp (Material); extend hit area beyond visual bounds if needed
...
| **Stack best practices** | "React performance tips"م€پ"SwiftUI navigation" | Step 4 (stack search) |
...
| Dark mode contrast issues | Quick Reference آ§6: `color-dark-mode` + `color-accessible-pairs` |
| Animations feel unnatural | Quick Reference آ§7: `spring-physics` + `easing` + `exit-faster-than-enter` |
| Form UX is poor | Quick Reference آ§8: `inline-validation` + `error-clarity` + `focus-management` |
| Navigation feels confusing | Quick Reference آ§9: `nav-hierarchy` + `bottom-nav-limit` + `back-behavior` |
| Layout breaks on small screens | Quick Reference آ§5: `mobile-first` + `breakpoint-consistency` |
| Performance / jank | Quick Reference آ§3: `virtualize-lists` + `main-thread-budget` + `debounce-throttle` |

### Pre-Delivery Checklist

- Run `--domain ux "animation accessibility z-index loading"` as a UX validation pass before implementation
- Run through Quick Reference **آ§1â€“آ§3** (CRITICAL + HIGH) as a final review
...
|------|----------|--------|----------------|
| **No Emoji as Structural Icons** | Use vector-based icons (e.g., Lucide, react-native-vector-icons, @expo/vector-icons). | Using emojis (ًںژ¨ ًںڑ€ âڑ™ï¸ڈ) for navigation, settings, or system controls. | Emojis are font-dependent, inconsistent across platforms, and cannot be controlled via design tokens. |
...
| **Touch Target Minimum** | Minimum 44أ—44pt interactive area (use hitSlop if icon is smaller). | Small icons without expanded tap area. | Meets accessibility and platform usability standards. |
